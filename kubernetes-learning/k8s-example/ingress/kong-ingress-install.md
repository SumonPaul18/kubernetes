# **Kong Ingress Controller (KIC)** এবং **Kong Gateway**-কে **K3s** বা **Kubernetes (K8s)** ক্লাস্টারে **বিগিনার লেভেল থেকে শুরু করে** ধাপে ধাপে ইন্সটল, কনফিগার এবং ব্যবহার করার সহজ গাইড দেওয়া হলো।  

---

## ✅ **প্রয়োজনীয় জিনিসপত্র (Prerequisites)**

1. **K3s বা K8s ক্লাস্টার** ইতিমধ্যে চলছে (Singel Node k3s)  
   → চেক করুন:  
   ```bash
   kubectl get nodes
   ```
...
2. **Check Helm 3** 
   ...
   helm version
   ```
   → If not installed, so install now - না থাকলে ইন্সটল করুন:
   ```bash
...
> 💡 `NodePort` ব্যবহার করা হচ্ছে কারণ আমি **আমার k3s cluster এ কোনো LoadBalancer (e.g. MetalLB) ব্যবহার করছি না** 
...
KIC + **cert-manager** দিয়ে automatic TLS করা যায়। 
...
## 🧩 **Gateway API ব্যবহার করতে চাইলে**
...
তারপর `Gateway`, `HTTPRoute` ব্যবহার করুন — এটা অ্যাডভান্সড টপিক.
